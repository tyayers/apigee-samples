...
sed -i "s,/:,/v1/samples/apihub-apigee-portal:,g" ./oas.local.yaml
sed -i "s,/ip:,/v1/samples/apihub-apigee-portal/ip:,g" ./oas.local.yaml
sed -i "s,/json:,/v1/samples/apihub-apigee-portal/json:,g" ./oas.local.yaml
...
# create portal doc
export CATALOG_ID=$(apigeecli apidocs create --allow-anon "true" --api-product "apihub-portal-product" --desc "Apigee Sample Product" --image-url "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQY6ifK6f2qRYyNCAPSwg9AFPN0jENtVzTRyw&s" -p "true" --require-callback-url "false" -l "Apigee Sample Product" -s "$APIGEE_PORTAL_ID" -o "$PROJECT_ID" -t $(gcloud auth print-access-token) | jq --raw-output ".data.id")
...
sed -i "s.-v1-deployment\".-v1-deployment\", \"projects/hub-migration-demo/locations/europe-west1/deployments/apigee-managed-api-v1-deployment\".g" ./apihub-api-version.local.json
...
curl -X POST "https://apihub.googleapis.com/v1/projects/$PROJECT_ID/locations/$REGION/deployments?deploymentId=apigee-managed-api-v1-deployment" \
